...
- 关于登录时报错空指针问题：原因在于插入数据时，每一条的数据不是从0开始的，而是从某一个数开始的。所以我们需要自己手动修改两张表的id字段。一个是s_menu表，将其id字段改为从1开始。第二个是s_user_role表，需要将其id改为role_id，保证id和role_id保持一致即可；修改id和role_id时还需要注意在role表中的数据。
